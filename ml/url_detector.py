...
def predict_url(url: str) -> Dict[str, Optional[object]]:
    ...
    try:
        ...
        result["error"] = "LEGACY UCI 30-FEATURE MODEL unavailable/conditional telemetry: " + str(error)
        result["legacy_model"] = True
        return result
    ...
    if artifact is None:
        result["error"] = (
            "The URL model artifact is unavailable. Add the bundled artifact or "
            "configure URL_MODEL_URL; no prediction was generated."
        )
        return result
    try:
