...
def _lexical_risk_assessment(raw_url: str) -> tuple[str, int, str, list[str]]:
    ...
    score += min(len(high_hits) * 12 + max(len(med_hits) - 1, 0) * 6, 24)
    ...
    if score >= 70:
        ...
        verdict, strength = "SUSPICIOUS", "low"
...
def analyze_url(url: str, *, include_enrichment: bool = True) -> dict:
    ...
    remote = remote_value if isinstance(remote_value, dict) and any(
        name in remote_value for name in ("virustotal", "urlscan", "urlhaus", "openphish")) else {}

    # Log whether collect_remote returned recognisable provider keys at all.
    log_scan_stage("remote_raw", normalized_url=normalized,
                   remote_type=type(remote_value).__name__,
                   remote_keys=list(remote_value.keys()) if isinstance(remote_value, dict) else [],
                   recognised_provider_keys=list(remote.keys()))

    # Preserve malformed adapter output as explicit error evidence.  A single
    # provider must never be able to abort aggregation of the other sources.
    provider_names = ("virustotal", "urlscan", "urlhaus", "openphish")
    fallback_reason = (remote_value.get("reason") if isinstance(remote_value, dict) else None) or "Provider did not return a result"
    providers = {name: item if isinstance(item, dict) else result(name, ERROR, reason="Provider returned malformed evidence")
                 for name, item in remote.items()}
    for name in provider_names:
        providers.setdefault(name, result(name, UNAVAILABLE, reason=fallback_reason))
    ...
    # URL scheme is the only input for the "does not use HTTPS" lexical
    # indicator.  A failed TLS connection is separately recorded as unavailable
    # evidence and must not be misreported as plain HTTP.
    local = explain_threat(None, similarity, tls.get("https", False), normalized, website.get("title"))
